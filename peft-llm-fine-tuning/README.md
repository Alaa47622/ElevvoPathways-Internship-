...
A complete portfolio-style implementation for **Task 9: Efficient LLM Fine-Tuning (PEFT)**.
...
> Important: the actual ROUGE values are generated by running the experiments. Do not put invented scores in your report.
...
```text
                    ...
                    └──────────┬───────────┘
                               │
                    ┌──────────▼───────────┐
                    │ Base Phi-2 evaluation│
                    └──────────┬───────────┘
                               │
                               ▼
                 ┌────────────────────────────┐
                 │ Phi-2 + 4-bit quantization │
                 └─────────────┬──────────────┘
                               │
                               ▼
                       ┌───────────────┐
...
Use the same test subset and generation settings for a fair comparison.
...
- Docker → reproducible serving
- FastAPI → inference service
- GitHub Actions → automated tests
- optional DVC → data/model versioning

## Dataset note

SAMSum is distributed under a non-commercial CC BY-NC-ND license. Check the dataset license before using it for commercial purposes.

## Model note

Phi-2 is released under the MIT license. Always review the current model card before redistribution/deployment.

## Important

This repository contains the complete *code and configuration*, but not the large model weights, trained adapter, or dataset. Those are downloaded/generated when you run the pipeline.

The actual benchmark results are also not pre-filled because they must come from your own run.
